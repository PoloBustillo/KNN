...
configs = [
    ...
    {"k_KNN": 5, "k_ENN": 7, "PCA_enabled": False, "ENN_enabled": True, "distance_metric": utilities.euclidean},
...
if __name__ == '__main__':
    ...
    dataPCA = utilities.apply_PCA(dataAndClasses, display)
    # Get correlation matrix (pearson) to get value closer to threshold
    matrixCorrelation = data.corr()
    xAttr = "0"
    yAttr = "1"
    if data.shape[1] == 2 and display:
        fig = px.scatter(dataAndClasses, x=str(xAttr), y=str(yAttr), symbol=classes, color=classes)
        fig.update_layout(coloraxis_colorbar=dict(yanchor="top", y=1, x=0, ticks="outside"))
        fig.show()
    else:
        values = utilities.getIndicesFromCorrelation(matrixCorrelation.abs().to_numpy(), corrT)
        dataFrequencies = []
        for value in values:
            dataFrequencies.append(len(set(data.iloc[:, value["i"]])) + len(set(data.iloc[:, value["j"]])))
    ...
    for config in configs:
        ...
        if config.get('ENN_enabled'):
            ...
            print(ennPlusData)
            fig1 = px.scatter(ennPlusData, x=str(xAttr), y=str(yAttr), symbol=classesOld, color=classesWithEnn)
            fig1.show()
